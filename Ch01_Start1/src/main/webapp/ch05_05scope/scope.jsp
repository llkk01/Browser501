...
pageContext Scope : 하나의 jsp페이지 안에서만 사용(변수) <br>
request Scope : include와 forward를 통해서 다른 페이지에서도 사용(변수) <br>
session Scope : Browser상태에서 페이지 이동시 사용(변수공유) <br>
application Scope : 서버가 시작부터 종료까지 사용(변수공유 : 방문자수) <br><hr>

<%
	pageContext.setAttribute("page", "page1"); //같은 페이지 내에서만 동작
	request.setAttribute("request", "request1"); // forward, include 있을때 동작
	session.setAttribute("session", "session1"); // 같은 브라우저 내에서만 동작
...
<!-- 전달만 해주는 것 -->
<%
	// request 영역에 데이터 저장시 forward, include를 통해서 사용
	RequestDispatcher d = request.getRequestDispatcher("other_scope.jsp");
	d.forward(request, response);
%>


<!-- RequestDispatcher > "other_scope.jsp"로 데이터가 넘어갔기때문에 아래는 실행이 안되고 "other_scope.jsp"가 실행 -->
